...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        if message.time_to_start <= current_time:
            PeriodicTask.objects.create(name=f'mailing list_{message}', task='send_mail_3_to_clients',
            ...
            PeriodicTask.objects.create(name=f'mailing list{message}', task='send_mail_3_to_clients',
                                        interval=IntervalSchedule.objects.get(every=message.custominterval.every,
                                                                              period=message.custominterval.period),
